...
describe('source dataset and portable schema', () => {
  it('contains the full trip, with reusable hotels and library entities', () => {
    ...
    expect(trip.entities.filter((e) => e.type === 'activity')).toHaveLength(70)
    ...
    expect(new Set(trip.days.flatMap((day) => day.items.map((item) => item.kind)))).toEqual(new Set(['drive', 'walk', 'food', 'visit', 'stay', 'flight', 'bike']))
