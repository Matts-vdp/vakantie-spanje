...
export function DayView({ trip, day, todayMode = false, onSaveNote, onDirty, onSaveTrip }: { onSaveTrip: SaveTrip; trip: Trip; day: Day; todayMode?: boolean; onSaveNote: (value: string) => Promise<void>; onDirty: (value: boolean) => void }) {
  ...
  return <div className="day-view">
    ...
    {hotel?.type === 'hotel' && <section className="hotel-panel">
      ...
      {hotel.navigationUrl && <div className="hotel-primary-action"><a className="action primary" href={hotel.navigationUrl} target="_blank" rel="noopener noreferrer"><Icon name="pin" size={16} />Navigate</a></div>}
      <details className="hotel-more"><summary>{checkoutOnly ? 'Departure & stay details' : 'More stay details & actions'}</summary><div className="hotel-secondary-actions">{hotel.websiteUrl && <a className="action" href={hotel.websiteUrl} target="_blank" rel="noopener noreferrer">Website ↗</a>}{hotel.phone && <a className="action" href={`tel:${hotel.phone.replace(/[^+\d]/g, '')}`}>Call</a>}<a className="action" href={`#/place/${hotel.id}`}>Hotel details</a>{stayContext && <a className="action" href={`#/stay/${stayContext.id}`}>Edit stay</a>}</div>{stayContext && <BookingDetails booking={stayContext.booking} />}{hotel.hotel.arrivalRequirements && <p>{hotel.hotel.arrivalRequirements}</p>}{hotel.notes && <p>{hotel.notes}</p>}<p className="small muted">{stayContext?.checkInDate} — {stayContext?.checkOutDate} · Checkout {hotel.hotel.checkOut || 'confirm with hotel'}</p>{hotel.hotel.dinner && <p className="small">Dinner: {hotel.hotel.dinner}</p>}</details>
    </section>}
    ...
    <section className="day-review" aria-labelledby="day-review-title"><div className="section-heading"><h2 id="day-review-title">To review</h2><a className="compact-add-action" href={`#/new/${day.id}`}><Icon name="plus" size={15} />Add a place or note</a></div><TodayReview trip={trip} dayId={day.id} /></section>
