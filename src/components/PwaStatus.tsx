...
export function PwaStatus({ busy }: { busy: boolean }) {
  ...
  return <aside className="pwa-message" aria-live="polite">
    ...
    {needRefresh && <button disabled={busy} onClick={() => { void updateServiceWorker(true).catch(() => setError('The update could not finish. Please try again while connected.')) }}>{busy ? 'Save your note first' : 'Update app'}</button>}
